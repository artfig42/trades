			...
			for (Entry<String, Position> entry : Position.positionMap.entrySet()) {
				...
				try (CSVPrinter posPrinter = CsvU.csvPrinter(tickerPath + pos.symbol + ".csv");) {
					...
					for (FidelityRecord fr : pos.historyList) {
						...
						localDates.add(fr.calDate);
						++cellNumber;
					}
					posPrinter.printRecord("A", "B", "C", "D", "E", "=SUM(F2:F" + cellNumber + ")",
							"=SUM(G2:G" + cellNumber + ")", "H", "=SUM(I2:I" + cellNumber + ")",
							"=SUM(J2:J" + cellNumber + ")", "=SUM(K2:K" + cellNumber + ")");

					for (LocalDate localDate : localDates) {
						posPrinter.printRecord(localDate);
					}
				}
			}
